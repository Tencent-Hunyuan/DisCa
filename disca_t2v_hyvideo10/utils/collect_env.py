...
def _collect_cuda_env(env_info):
    ...
    if cuda_home is None or not osp.isdir(cuda_home):
        return
...
def _collect_musa_env(env_info):
    devices = defaultdict(list)
    ...
    if musa_home is None or not osp.isdir(musa_home):
        return
...
def _collect_compiler_env(env_info, distutils_errors):
    import io
    ...
    try:
        ...
        env_info['GCC'] = 'n/a'
    except (subprocess.CalledProcessError, distutils_errors.DistutilsPlatformError):
        env_info['GCC'] = 'n/a'
...
def _collect_optional_package_env(env_info):
    try:
        import torchvision
...
def collect_env():
    ...
    if cuda_available:
        _collect_cuda_env(env_info)
    elif musa_available:
        _collect_musa_env(env_info)

    _collect_compiler_env(env_info, errors)

    env_info['PyTorch'] = torch.__version__
    env_info['PyTorch compiling details'] = get_build_config()
    _collect_optional_package_env(env_info)
